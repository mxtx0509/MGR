...
class SRDataset(data.Dataset):
    def __init__(self, adj_dir, feature_dir, file_list,adj_size=8,graph_mode='pose',is_train = True):
        ...
        self.graph_perobj_dir = '/export/home/zm/test/icme2019/pygcn/graph/'
        self.graph_pose_dir = '/export/home/zm/test/icme2019/pygcn/graph_pose/'
    ...
    def __getitem__(self, index):
        ...
        # bbox1_pose_mean = np.load('/export/home/zm/test/icme2019/pose_embedding/PISC_256_mean/' + feature_name1)
        
        # bbox2_pose = np.load(self.pose_dir + feature_name2)
        bbox2_location = np.load(self.location_dir + feature_name2)
        bbox2_visual = np.load(self.visual_dir + feature_name2)
        # bbox2_pose_mean = np.load('/export/home/zm/test/icme2019/pose_embedding/PISC_256_mean/' + feature_name2)
        
        pose_feature[0:17] = bbox1_location
        pose_feature[17:34] = bbox2_location
        
        union_visual = np.load(self.visual_dir + union_name)
        # pose_feature1 = np.concatenate((bbox1_pose,bbox1_location),axis=1) 
        # pose_feature2 = np.concatenate((bbox2_pose,bbox2_location),axis=1) 
        ...
        # graph_feature[8,0:256] = bbox1_pose_mean
        # graph_feature[9,0:256] = bbox2_pose_mean
        # graph_feature[10:27,0:30] = bbox1_location #pose_feature1
        # graph_feature[27:44,0:30] = bbox2_location

 
        # if label==4 or label==2:
        # a = random.random()
        # if a >0.5 and self.is_train == True:
            # graph_feature[0] = bbox2_visual
            # graph_feature[1] = bbox1_visual
            # graph_feature[8,:] = bbox2_pose_mean
            # graph_feature[9,:] = bbox1_pose_mean
            # graph_feature[10:27,0:686] = pose_feature2
            # graph_feature[27:44,0:686] = pose_feature1
        ...
        # adj_npy = np.load(self.graph_dir + file_id + '_%d_%d_%d_%d.npy'%(box1[0],box1[1],box2[0],box2[1]))
        # adj = np.zeros((37,37))
        # adj[0:3,0:3]=1
        # adj[3:,3:] = adj_npy
