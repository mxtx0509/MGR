...
def main():
    ...
    model = GCN(adj_size=SIZE,nfeat=256,nhid=args.hidden,nclass=args.num_class,dropout=args.dropout) 
    # print model
...
def validate(val_loader, model, criterion, fnames=[]):
    ...
    p = {}  # prediction
    r = {}  # recall
    output_sum = np.zeros((5106,args.num_class))
    for i, (adj_tensor, feature_tensor,target) in enumerate(val_loader):
        ...
        losses.update(loss.data[0], feature_tensor.size(0))
        prec1 = accuracy(output.data, target)
        top1.update(prec1[0], feature_tensor.size(0))
